...
async function loadMovies() {
    ...
    try {
        ...
        snap.forEach(function(d) {
            window.moviesState.movies.push({ id: d.id, data: d.data(), _firestoreId: d.id });
        });
        // Раскрываем data
        window.moviesState.movies = window.moviesState.movies.map(function(m) {
            return Object.assign({}, m.data, { _firestoreId: m._firestoreId });
        });
        window.moviesState.movies.sort(function(a, b) {
...
function updateMoviesCounters() {
    ...
    var el = document.getElementById('moviesAllCount');
    if (el) el.textContent = all.length;
    el = document.getElementById('moviesWantCount');
    if (el) el.textContent = all.filter(function(m) { return m.status === 'want'; }).length;
    el = document.getElementById('moviesWatchedCount');
    if (el) el.textContent = all.filter(function(m) { return m.status === 'watched' || m.status === 'favourite'; }).length;
    el = document.getElementById('moviesFavCount');
    if (el) el.textContent = all.filter(function(m) { return m.status === 'favourite'; }).length;
}
...
async function getKinopoiskDetails(filmId) {
    try {
        var resp = await fetch('https://kinopoiskapiunofficial.tech/api/v2.2/films/' + filmId, {
            headers: { 'X-API-KEY': window.KINOPOISK_API_KEY }
        });
...
window.renderMoviesSection = function(container) {
    ...
    document.getElementById('moviesAddBtn').addEventListener('click', function() {
        var query = prompt('🔍 Введите название фильма для поиска на Кинопоиске:');
...
async function searchAndShowKP(query) {
    ...
    container.innerHTML = results.slice(0, 6).map(function(f) {
        return '<div class="col-md-4 col-lg-2"><div class="card h-100" style="cursor:pointer" onclick="window.addMovieFromKP(' + f.filmId + ')">' +
...
window.addMovieFromKP = async function(filmId) {
    ...
    if (!details) return alert('Не удалось загрузить данные');
    ...
    var data = {
        ...
        genres: details.genres ? details.genres.map(function(g) { return g.genre; }) : [],
        runtime: details.filmLength || 0,
        slogan: details.slogan || '',
        countries: details.countries ? details.countries.map(function(c) { return c.country; }) : [],
        status: 'want',
