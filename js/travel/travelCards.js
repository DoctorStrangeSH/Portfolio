    ...
    col.innerHTML = `
        <div class="card h-100 shadow-sm travel-card-v2 mb-3 border-0 overflow-hidden" style="border-left:5px solid ${season ? season.color : '#dee2e6'}">
            <div class="position-relative">
                ...
                </div>` : `<img src="${mainPhoto}" class="card-img-top" style="height:220px;object-fit:cover" ${photos.length>0?`onclick="window.openGallery(${photosJson},0)" style="cursor:pointer"`:''}>`}
...
// ========== МОДАЛКА ДОБАВЛЕНИЯ ==========
window.showTravelAddModal = function() {
    const old = document.getElementById('travelAddModal');
    if (old) old.remove();
    document.body.insertAdjacentHTML('beforeend', `
        <div class="modal fade" id="travelAddModal" tabindex="-1">
            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content border-0 shadow">
                    <div class="modal-header bg-light"><h5 class="modal-title"><i class="bi bi-plus-circle me-2"></i>Новое место</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    <div class="modal-body" id="travelAddModalBody"></div>
                </div>
            </div>
        </div>`);
    
    const body = document.getElementById('travelAddModalBody');
    window._travelDiary = [];
    body.innerHTML = `
        <form id="travelAddForm" autocomplete="off">
            ...
            <div id="taVisitedFields" class="d-none"><div class="row g-3 mb-3"><div class="col-md-6"><label class="form-label fw-medium">📅 Дата</label><input type="date" class="form-control" id="taDate"></div><div class="col-md-6"><label class="form-label fw-medium">⭐ Оценка</label><div class="d-flex gap-1" id="taStars"></div></div></div></div>
    ...
    const modal = new bootstrap.Modal(document.getElementById('travelAddModal'));
    modal.show();
    
    document.getElementById('travelAddForm').onsubmit = async (e) => {
        e.preventDefault();
        const name = document.getElementById('taName').value.trim();
        if (!name) return alert('Введите название!');
        const status = document.getElementById('taStatus').value;
        const pr = document.getElementById('taPhotos').value.trim();
        const tr = document.getElementById('taTags').value.trim();
        const data = {
            name, location: document.getElementById('taLocation').value.trim(),
            ...
            album: document.getElementById('taAlbum').value.trim(),
            description: document.getElementById('taDesc').value.trim(),
            tags: tr ? tr.split(',').map(s=>s.trim().toLowerCase()).filter(s=>s) : [],
            status, date: status==='visited' ? document.getElementById('taDate').value : '',
